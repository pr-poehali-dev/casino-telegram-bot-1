...
# Сумма, начиная с которой требуется подтверждённый телефон (защита крупных выводов)
PHONE_VERIFY_THRESHOLD = 25_000
...
def handler(event: dict, context) -> dict:
    ...
    if method_http == 'GET' and params.get('action') == 'limits':
        ...
        return {'statusCode': 200, 'headers': HEADERS, 'body': json.dumps({
            ...
            'phone_verify_threshold': PHONE_VERIFY_THRESHOLD,
    ...
    if user:
        ...
        # Крупные выводы требуют подтверждённого телефона (защита от фрода)
        if amount >= PHONE_VERIFY_THRESHOLD and not phone_verified:
            cur.close(); conn.close()
            return {'statusCode': 403, 'headers': HEADERS,
                    'body': json.dumps({
                        'error': 'phone_not_verified',
                        'message': f'Для вывода от {PHONE_VERIFY_THRESHOLD:,} ₽ подтверди номер телефона'.replace(',', ' ')
                    }), 'isBase64Encoded': False}
